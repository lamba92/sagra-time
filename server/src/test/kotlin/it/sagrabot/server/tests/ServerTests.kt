...
class ServerTests {
    ...
    @Test
    fun insertAndRetrieve() = testApplication {
        client = createClient {
        ...
        application { SagraBot(store) }
        val postResult = client.post("api/v1/sagra/add") {
            header(HttpHeaders.ContentType, ContentType.Application.Json)
            setBody(TestSagra)
        }
        if (postResult.status.value != 200) error("Failed to insert test sagra")
        val result = client.get("api/v1/sagra/all").body<List<Sagra>>()
        assertEquals(listOf(TestSagra), result)
    }
}

private val TestSagra =
    Sagra(
        name = "test",
        food = listOf("test1", "test2"),
        from = LocalDateTime.parse(Clock.System.now().toString().dropLast(1)),
        until = LocalDateTime.parse((Clock.System.now() + 3.days).toString().dropLast(1)),
        description = "test",
        location = Location(
            lat = 1.0,
            lon = 2.0,
