            ...
            effect(() => {
                ...
                if (isPlatformBrowser(inject(PLATFORM_ID))) {
                    try {
                        ...
                        console.log("Failed to save theme mode to localStorage:")
                    }
                    try {
                        const maxAge = 60 * 60 * 24 * 365; // 1 year
                        document.cookie = `theme-mode=${m}; Path=/; Max-Age=${maxAge}`;
                    } catch {
                        console.log("Failed to save theme mode to cookie")
                    }
                }
                this.updateIsDark();
                this.applyTheme();
    ...
    private setupMediaQuery() {
        ...
        try {
            ...
            try {
                ...
                console.warn('Failed to set up system theme listener');
            }
        }
    }
